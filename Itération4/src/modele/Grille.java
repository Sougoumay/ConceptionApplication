...
public class Grille extends Plateau {
    ...
    public boolean partieTermine(String jeton)
    {
        return  aligner(jeton, 4) == true;
    }

    public boolean aligner(String jeton, int nbreAlig) {
        ...
        for (int ligne = 0; ligne < taille; ligne++) {
            ...
            if (chercherAlignement(0, ligne, 1, 0,jeton,nbreAlig)) {
                return true;
            }
        }
        ...
        for (int col = 0; col < taille; col++) {
            ...
            if (chercherAlignement(col, 0, 0, 1,jeton,nbreAlig)) {
                return true;
            }
        }
        ...
        for (int col = 0; col < taille; col++) {
            // Première diagonale ( \ )
            if (chercherAlignement(col, 0, 1, 1,jeton,nbreAlig)) {
                return true;
            }
            // Deuxième diagonale ( / )
            if (chercherAlignement(col, 0, -1, 1,jeton,nbreAlig)) {
        ...
        for (int ligne = 0; ligne < taille; ligne++) {
            // Première diagonale ( / )
            if (chercherAlignement(0, ligne, 1, 1,jeton,nbreAlig)) {
                return true;
            }
            // Deuxième diagonale ( \ )
            if (chercherAlignement(taille - 1, ligne, -1, 1,jeton,nbreAlig)) {
                return true;
            }
        }
        // On n'a rien trouvé
    ...
    private boolean chercherAlignement(int colonne, int ligne, int modifColonne, int modifLigne, String jeton, int nbreAlig) {
        String couleur = jeton;
        int compteur = 0;
    ...
    public void gererCoup(Coup coup) throws CoupInvalideException {
        ...
        String jeton = ((CoupPuissance)coup).getJeton();
    ...
    public void setGrille(int ligne, int colonne, String jeton) {
